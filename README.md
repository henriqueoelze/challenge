...
2. Some validations maybe need to be changes. I implemented using the PDF as guide, and for example `"There should not be more than 2 similar transactions (same amount and merchant) in a 2 minutes interval"` is saying "more than 2", so the validation is allowing 2 similiar transactions and only return error when receive the third one. Maybe this was a miss understand of the feature, but I think it is important to mention here.
